...
for filename in glob.glob("*_rr.csv"):
        ...
        if y in df.index.year:  # Checks if values for given year exist
            ...
            for m in range(1, 13):
                if m in df_year.index.month:
                    ...
                    cwd_df = df_year[df_year.index.month == m][df_year[df_year.index.month == m] >= 1].dropna()
    ...
    df_output = pd.DataFrame({"Date": dates, "CWD per month (# of days)": cwd_lst})
    df_output.to_csv("CWD_monthly_" + filename[:5] + ".csv", sep=";", index=False)
