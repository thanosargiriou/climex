...
"""
Climate index: R95p
Definition: [Annual total PRCP when PR > 95th percentile] Let RRwj be the daily precipitation amount on a wet day
...
variable = "precip(mm)"  # Variable under calculation
...
for filename in glob.glob("*_rr.csv"):
    ...
    df = df_import[df_import > 1].dropna()  # Wet days dataframe
    df_base_period = df[(df.index.year >= base_start) & (df.index.year <= base_end)]  # Data for base period
    r99p = []  # List to append calculated percentile
    ...
    for m in range(1, 13):  # Iteration over months
        ...
        for d in range(1, 32):  # Iteration over days
            ...
            if d not in df_month.index.day:  # Needed in order to avoid index errors caused by values that do not exist
                ...
                r99p.append(percentile[0])  # Adding calculated percentile to corresponding list
    ...
    for i in year_lst:
        ...
        if leap_year(i) == "True":
            ...
            for check_day in range(len(df_not_in_base_y.index)):  # Iteration over year dataframe
                ...
                if df_not_in_base_y[df_not_in_base_y.index.dayofyear == doy][variable][0] > r99p[doy - 1]:
                    count = count + 1
                else:
                    pass
        else:
            count = 0
            r99p_not_leap = r99p[:59] + r99p[60:]
            for check_day in range(len(df_not_in_base_y.index)):  # Iteration over year dataframe
                doy = df_not_in_base_y.index[check_day].dayofyear  # Day of the year starting 1 to 165-366
                # -1 because python numbering starts from 0
                if df_not_in_base_y[df_not_in_base_y.index.dayofyear == doy][variable][0] > r99p_not_leap[doy - 1]:
                    count = count + 1
                else:
                    pass
    ...
    df_output_out_of_base = pd.DataFrame({"Date": dates, "r99p (% of days)": r_lst_final})
    ...
    # Iteration over base period years
    for o in range(df_base_period.index.year[0], df_base_period.index.year[-1] + 1):
        ...
        for p in iter_years:  # Iteration over 29 remaining base period years
            ...
            r99p_bootstrap = []  # List to append calculated percentile
            ...
            for m in range(1, 13):  # Iteration over months
                ...
                for d in range(1, 32):  # Iteration over days
                    ...
                    if d not in df_m.index.day:
                        ...
                        r99p_bootstrap.append(percentile[0])  # Adding calculated percentile to corresponding list
            ...
            if leap_year(o) == "True":
                ...
                for check in range(len(df_bootstrap_one_year.index)):  # Iteration over year dataframe
                    ...
                    if df_bootstrap_one_year[df_bootstrap_one_year.index.dayofyear == day][variable][0] > \
                            r99p_bootstrap[day - 1]:
                        counter = counter + 1
                    else:
                        pass
            else:
                counter = 0
                r99p_no_leap = r99p_bootstrap[:59] + r99p_bootstrap[60:]
                for check in range(len(df_bootstrap_one_year.index)):  # Iteration over year dataframe
    ...
    df_output_base_period = pd.DataFrame({"Date": base_year_lst, "r99p (% of days)": index_base_list})
    ...
    df_output.to_csv("r99p_" + filename[:5] + ".csv", sep=";", index=False, float_format="%.2f")
