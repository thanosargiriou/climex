...
"""
Climate index: TX90p
Definition: [Percentage of days when TX > 90th percentile] Let TXij be the daily maximum temperature on day i
            in period j and let TXin90 be the calendar day 90th percentile centred on a 5-day window
...
for filename in glob.glob("*_TX_d.csv"):
    ...
    tx90p = []  # List to append calculated percentile
    ...
    for m in range(1, 13):  # Iteration over months
        ...
        for d in range(1, 32):  # Iteration over days
            ...
            if d not in df_month.index.day:  # Needed in order to avoid index errors caused by values that do not exist
                ...
                tx90p.append(percentile[0])  # Adding calculated percentile to corresponding list
    ...
    tx_lst_final = []  # list to append calculated indexes
    dates = []
    ...
    for i in year_lst:
        ...
        if leap_year(i) == "True":
            ...
            for check_day in range(len(df_not_in_base_y.index)):  # Iteration over year dataframe
                ...
                if df_not_in_base_y[df_not_in_base_y.index.dayofyear == doy][variable][0] > tx90p[doy - 1]:
                    count = count + 1
                else:
                    pass
        else:
            count = 0
            tx90p_not_leap = tx90p[:59] + tx90p[60:]
            for check_day in range(len(df_not_in_base_y.index)):  # Iteration over year dataframe
                doy = df_not_in_base_y.index[check_day].dayofyear  # Day of the year starting 1 to 165-366
                # -1 because python numbering starts from 0
                if df_not_in_base_y[df_not_in_base_y.index.dayofyear == doy][variable][0] > tx90p_not_leap[doy - 1]:
                    count = count + 1
                else:
                    pass

        count = (count / len(df_not_in_base_y.index)) * 100
        tx_lst_final.append(count)
        dates.append(df_not_in_base_y.index.year[0])

    # Dataframe with index values for out of base period data
    df_output_out_of_base = pd.DataFrame({"Date": dates, "TX90p (% of days)": tx_lst_final})
    ...
    for o in range(df_base_period.index.year[0], df_base_period.index.year[-1] + 1):  # Iteration over base period years
        ...
        for p in iter_years:  # Iteration over 29 remaining base period years
            ...
            tx90p_bootstrap = []  # List to append calculated percentile
            ...
            for m in range(1, 13):  # Iteration over months
                ...
                for d in range(1, 32):  # Iteration over days
                    ...
                    if d not in df_m.index.day:
                        ...
                        tx90p_bootstrap.append(percentile[0])  # Adding calculated percentile to corresponding list
            ...
            if leap_year(o) == "True":
                ...
                for check in range(len(df_bootstrap_one_year.index)):  # Iteration over year dataframe
                    ...
                    if df_bootstrap_one_year[df_bootstrap_one_year.index.dayofyear == day][variable][0] > \
                            tx90p_bootstrap[day - 1]:
                        counter = counter + 1
                    else:
                        pass
            else:
                counter = 0
                tx90p_no_leap = tx90p_bootstrap[:59] + tx90p_bootstrap[60:]
                for check in range(len(df_bootstrap_one_year.index)):  # Iteration over year dataframe
    ...
    df_output_base_period = pd.DataFrame({"Date": base_year_lst, "TX90p (% of days)": index_base_list})
    ...
    df_output.to_csv("tx90p_" + filename[:5] + ".csv", sep=";", index=False, float_format="%.2f")
