...
"""
Climate index: TN10p
Definition: [Percentage of days when TN < 10th percentile] Let TNij be the daily minimum temperature on day i
            in period j and let TNin10 be the calendar day 10th percentile centred on a 5-day window
...
variable = "TN"  # Variable under calculation
...
for filename in glob.glob("*_TN_d.csv"):
    ...
    tn10p = []  # List to append calculated percentile
    ...
    for m in range(1, 13):  # Iteration over months
        ...
        for d in range(1, 32):  # Iteration over days
            ...
            if d not in df_month.index.day:  # Needed in order to avoid index errors caused by values that do not exist
                ...
                tn10p.append(percentile[0])  # Adding calculated percentile to corresponding list
    ...
    for i in year_lst:
        ...
        if leap_year(i) == "True":
            ...
            for check_day in range(len(df_not_in_base_y.index)):  # Iteration over year dataframe
                ...
                if df_not_in_base_y[df_not_in_base_y.index.dayofyear == doy][variable][0] < tn10p[doy - 1]:
                    count = count + 1
                else:
                    pass
        else:
            count = 0
            tn10p_not_leap = tn10p[:59] + tn10p[60:]
            for check_day in range(len(df_not_in_base_y.index)):  # Iteration over year dataframe
                doy = df_not_in_base_y.index[check_day].dayofyear  # Day of the year starting 1 to 165-366
                # -1 because python numbering starts from 0
                if df_not_in_base_y[df_not_in_base_y.index.dayofyear == doy][variable][0] < tn10p_not_leap[doy - 1]:
                    count = count + 1
                else:
                    pass
    ...
    df_output_out_of_base = pd.DataFrame({"Date": dates, "tn10p (% of days)": tn_lst_final})
    ...
    for o in range(df_base_period.index.year[0], df_base_period.index.year[-1] + 1):  # Iteration over base period years
        ...
        for p in iter_years:  # Iteration over 29 remaining base period years
            ...
            tn10p_bootstrap = []  # List to append calculated percentile
            ...
            for m in range(1, 13):  # Iteration over months
                ...
                for d in range(1, 32):  # Iteration over days
                    ...
                    if d not in df_m.index.day:
                        ...
                        tn10p_bootstrap.append(percentile[0])  # Adding calculated percentile to corresponding list
            ...
            if leap_year(o) == "True":
                ...
                for check in range(len(df_bootstrap_one_year.index)):  # Iteration over year dataframe
                    ...
                    if df_bootstrap_one_year[df_bootstrap_one_year.index.dayofyear == day][variable][0] < \
                            tn10p_bootstrap[day - 1]:
                        counter = counter + 1
                    else:
                        pass
            else:
                counter = 0
                tn10p_no_leap = tn10p_bootstrap[:59] + tn10p_bootstrap[60:]
                for check in range(len(df_bootstrap_one_year.index)):  # Iteration over year dataframe
    ...
    df_output_base_period = pd.DataFrame({"Date": base_year_lst, "tn10p (% of days)": index_base_list})
    ...
    df_output.to_csv("tn10p_" + filename[:5] + ".csv", sep=";", index=False, float_format="%.2f")
