...
"""
Climate index: TNn
Definition: [Min Tmin]. Monthly minimum value of daily minimum temperature.
...
for filename in glob.glob("*_TN_d.csv"):
    tnn_lst = []  # List to save index values per year
        ...
        if y in df.index.year:  # Checks if values for given year exist
            for m in range(1, 13):  # Iteration over months
                if m in df[df.index.year == y].index.month:  # Checks if values for given month exist
                    ...
                    tnn = df[df.index.year == y][df[df.index.year == y].index.month == m][df.columns[0]].min()
                    tnn_lst.append(tnn)
                else:
                    tnn = float("NaN")
                    tnn_lst.append(tnn)
    ...
    df_output = pd.DataFrame({"Date": dates, "TNn (°C)": tnn_lst})
    df_output.to_csv("tnn_" + filename[:5] + ".csv", sep=";", index=False)
