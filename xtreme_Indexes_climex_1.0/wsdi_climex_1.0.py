...
"""
Climate index: WSDI
Definition: [Warm spell duration index] Annual count of days with at least 6 consecutive days when TX > 90th percentile.
             Let TXij be the daily maximum temperature on day i in period j and let TXin90 be the calendar day
             90th percentile centred on a 5-day window for the base period 1961-1990. Then the number of days
             per period is summed where, in intervals of at least 6 consecutive days: TXij > TXin90
...
variable = "TX"  # Variable under calculation
...
for filename in glob.glob("*_TX_d.csv"):
    ...
    wsdi = []  # List to append calculated percentile
    ...
    for m in range(1, 13):  # Iteration over months
        ...
        for d in range(1, 32):  # Iteration over days
            ...
            if d not in df_month.index.day:  # Needed in order to avoid index errors caused by values that do not exist
                ...
                wsdi.append(percentile[0])  # Adding calculated percentile to corresponding list
    ...
    wsdi_lst_final = []  # list to append calculated indexes
    dates = []
    ...
    for i in year_lst:
        ...
        if leap_year(i) == "True":
            ...
            for check_day in range(len(df_not_in_base_y.index)):  # Iteration over year dataframe
                ...
                if df_not_in_base_y[df_not_in_base_y.index.dayofyear == doy][variable][0] > wsdi[doy - 1]:
            ...
            wsdi_not_leap = wsdi[:59] + wsdi[60:]
            for check_day in range(len(df_not_in_base_y.index)):  # Iteration over year dataframe
                doy = df_not_in_base_y.index[check_day].dayofyear  # Day of the year starting 1 to 165-366
                # -1 because python numbering starts from 0
                if df_not_in_base_y[df_not_in_base_y.index.dayofyear == doy][variable][0] > wsdi_not_leap[doy - 1]:
                    consecutive.append(df_not_in_base_y[df_not_in_base_y.index.dayofyear == doy].index)
        ...
        wsdi_lst_final.append(count)
        dates.append(df_not_in_base_y.index.year[0])

    # Dataframe with index values for out of base period data
    df_output_out_of_base = pd.DataFrame({"Date": dates, "WSDI (# of days)": wsdi_lst_final})
    ...
    for o in range(df_base_period.index.year[0], df_base_period.index.year[-1] + 1):  # Iteration over base period years
        ...
        for p in iter_years:  # Iteration over 29 remaining base period years
            ...
            wsdi_bootstrap = []  # List to append calculated percentile
            ...
            for m in range(1, 13):  # Iteration over months
                ...
                for d in range(1, 32):  # Iteration over days
                    ...
                    if d not in df_m.index.day:
                        ...
                        wsdi_bootstrap.append(percentile[0])  # Adding calculated percentile to corresponding list
            ...
            if leap_year(o) == "True":
                ...
                wsdi_no_leap = wsdi_bootstrap[:59] + wsdi_bootstrap[60:]
                for check in range(len(df_bootstrap_one_year.index)):  # Iteration over year dataframe
    ...
    df_output_base_period = pd.DataFrame({"Date": base_year_lst, "WSDI (# of days)": index_base_list})
    ...
    df_output.to_csv("wsdi_" + filename[:5] + ".csv", sep=";", index=False)
